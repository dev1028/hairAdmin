...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		$("#btnPrimaryCode").on("click", function(){
			...
			if(btnPrimaryCodeCnt == 0){
				...
				if($("#primary_codeInsertInput").val() != null
					...
					$.ajax("${pageContext.request.contextPath}/ajax/primaryCodeInsert.do", {
		...
		$("#btnSecondaryCode").on("click", function(){
			...
			$.ajax("${pageContext.request.contextPath}/ajax/secondaryCodeInsert.do", {
		...
		$("#codelisttable_tbody").on("click", ".btncodeDelete", function(){
			var $tr = $(this).closest("tr");
				$.ajax({url : "${pageContext.request.contextPath}/ajax/categoryMajorDelete.do", // 클라이언트가 요청을 보낼 서버의 URL 주소
		...
		$("#codelisttable_tbody").on("click", "#btnUpdateFin", function(){
			...
			$.ajax({
				url : "${pageContext.request.contextPath}/ajax/codeUpdate.do",
...
<body>
	...
	<br>
		...
		<hr>
		<form id="ischoosefrm" action="${pageContext.request.contextPath}/admin/codeList.do">
